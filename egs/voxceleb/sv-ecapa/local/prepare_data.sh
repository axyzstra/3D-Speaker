...
stage=-1
stop_stage=-1
data=/home/pod/shared-nvme/dataset
data_scp=/home/pod/shared-nvme/dataset/scp
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  # and vox2_aac.zip.
  echo "Download musan.tar.gz, rirs_noises.zip, vox1_test_wav.zip, vox1_dev_wav.zip."
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for archive in musan.tar.gz vox1_test_wav.zip vox1_dev_wav.zip; do
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  # mkdir -p ${data}/musan ${data}/vox1
  mkdir -p ${data_scp}/musan ${data_scp}/rirs ${data_scp}/vox1
  # musan
  # find $(pwd)/${rawdata_dir}/musan/noise/free-sound -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' >${data}/musan/wav.scp
  find ${rawdata_dir}/musan/noise/free-sound -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' >${data_scp}/musan/wav.scp
  # rirs
  ...
  awk '{print $5}' ${rawdata_dir}/RIRS_NOISES/real_rirs_isotropic_noises/rir_list | xargs -I {} echo {} ${rawdata_dir}/{} > ${data_scp}/rirs/wav.scp
  # vox1
  # find $(pwd)/${rawdata_dir}/voxceleb1 -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data}/vox1/wav.scp
  find ${rawdata_dir}/voxceleb1 -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data_scp}/vox1/wav.scp
  awk '{print $1}' ${data_scp}/vox1/wav.scp | awk -F "/" '{print $0,$1}' >${data_scp}/vox1/utt2spk
  ./utils/utt2spk_to_spk2utt.pl ${data_scp}/vox1/utt2spk >${data_scp}/vox1/spk2utt
  if [ ! -d ${data_scp}/vox1/trials ]; then
    echo "Download trials for vox1 ..."
    mkdir -p ${data_scp}/vox1/trials
