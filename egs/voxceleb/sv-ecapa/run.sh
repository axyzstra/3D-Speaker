...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  python local/prepare_data_csv.py --data_dir $data/scp/vox1
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu speakerlab/bin/train.py --config conf/ecapa_tdnn.yaml --gpu $gpus \
           --data $data/vox1/train.csv --noise $data/musan/wav.scp --reverb $data/rirs/wav.scp --exp_dir $exp_dir
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  torchrun --nproc_per_node=$num_gpu --master_port=29531 speakerlab/bin/extract.py --exp_dir $exp_dir \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  # 只在干净语音上进行评估  $data/vox1/trials/vox1_E_cleaned.trial $data/vox1/trials/vox1_H_cleaned.trial
